...
load_dotenv()
# Try multiple ways to get the API key (handle BOM issues)
API_KEY: Optional[str] = None
for key_name in ["GROUNDX_API_KEY", "\ufeffGROUNDX_API_KEY"]:
...
def _get_api_key():
    """Get API key from environment, handling BOM and reloading"""
...
def create_client(_api_key_hash: str = None) -> GroundX:
    """Initialize and return a Ground X API client instance
    
    Args:
        _api_key_hash: Hash of the API key to invalidate cache when key changes
    """
    # Get current API key (always fresh)
    api_key = _get_api_key()
    
    if not api_key or api_key == "your-groundx-api-key-here":
        raise ValueError("No `GROUNDX_API_KEY` found in secrets, .env, or environment. Please add your API key to the .env file.")
    
    try:
        return GroundX(api_key=api_key)
    except Exception as e:
        raise ValueError(f"Failed to create Ground X client: {str(e)}. Please check your API key.")
